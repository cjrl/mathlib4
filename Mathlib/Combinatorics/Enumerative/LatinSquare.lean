...
theorem LatinRectangle.exists_isSubrect_of_card_eq_card_add_one {k n : Type*} [Fintype n]
  ...
  have exactly_n_minus_k_cols_without_x : ∀ x,
    ...
    let g : Cs -> As := fun x => ⟨f' x, by
      exact Finset.mem_filter.mpr ⟨Finset.mem_univ _, ↑x, by simp [Matrix.col_apply, hf]⟩⟩
    ...
    have gsurj : Function.Surjective g := by
      have As_is_f_image : As = Finset.image f Finset.univ := by
        ext b; simp [As, Finset.mem_image, Matrix.col_apply, hf]
    ...
    have h_intersect : As ∩ Bs = ∅ := by
      ext a
      simp only [Finset.mem_inter, Finset.mem_empty, iff_false]
      intro ⟨haAs, haBs⟩
      have hInA : ∃ i, A.col a i = x := (Finset.mem_filter.mp haAs).2
      have hNotInA : x ∉ Finset.image (A.col a) Finset.univ := by
        have := (Finset.mem_filter.mp haBs).2
        simp only [B, symbolsNotIn] at this
        exact (Finset.mem_sdiff.mp this).2
      exact hNotInA (Finset.mem_image.mpr ⟨hInA.choose, Finset.mem_univ _, hInA.choose_spec⟩)
    have h_union_card : Finset.card (As ∪ Bs) = Fintype.card n := by
      have hunion : As ∪ Bs = Finset.univ := by
        ...
        · exact Or.inr (Finset.mem_filter.mpr ⟨Finset.mem_univ _, by
            simp only [B, symbolsNotIn, Finset.mem_sdiff, Finset.mem_univ, true_and,
                       Finset.mem_image, not_exists, Matrix.col_apply]
            exact h⟩)
  ...
  let A' : LatinRectangle k' n α := {
    ...
    row_injective := by
      ...
      · rename_i if_h₁
        ...
        apply h
      · simp only [Subtype.forall, Finset.mem_univ, forall_true_left, Set.mem_ofPred_eq] at hf
